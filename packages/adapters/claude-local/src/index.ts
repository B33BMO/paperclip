...
- permissionPrompts (string, optional, default "off"): "board" makes Claude ask the board before any tool use its rules do not allow, instead of bypassing permissions. Each request appears as an Allow once / Deny card on the task and the run waits for the decision. Every Bash command, file write/edit, web fetch, and connection tool asks; file reads (Read/Grep/Glob) ask unless they stay inside the run's own workspace, and Paperclip's own task tools never ask. Forces dangerouslySkipPermissions off, ignores permission-related extraArgs, and loads only user-level Claude settings. Requires engine="cli"; ACP is rejected. Works with filesystemScope/networkScope="allowlist" (not networkScope="deny", which cannot reach Paperclip, so everything that needs approval is denied).
- approvalTimeoutSec (number, optional, default 1800, 60-10800): how long a board prompt waits for a decision before the tool use is denied.
- dangerouslySkipPermissions (boolean, optional, default true; ignored when permissionPrompts="board"): allow non-interactive Claude runs to proceed without approval prompts. Local and remote targets receive --dangerously-skip-permissions for all built-in and connected tools. Managed sandbox targets also identify themselves to Claude so root container launches support bypass. Non-sandbox root processes must run Claude as a non-root user; Paperclip does not silently downgrade the requested mode.
