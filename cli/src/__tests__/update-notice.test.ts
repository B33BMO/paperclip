...
describe("update notice", () => {
  it("is disabled by default and opt-in via env or config", () => {
    expect(isUpdateNoticeEnabled(path.join(root, "missing.json"))).toBe(false);
    process.env.PAPERCLIP_UPDATE_CHECK = "1"; expect(isUpdateNoticeEnabled(path.join(root, "missing.json"))).toBe(true);
    delete process.env.PAPERCLIP_UPDATE_CHECK; const config = path.join(root, "enabled.json"); fs.writeFileSync(config, JSON.stringify({ updates: { checkEnabled: true } })); expect(isUpdateNoticeEnabled(config)).toBe(true);
  });
  ...
  it("throttles registry checks for 24 hours", async () => {
    process.env.PAPERCLIP_UPDATE_CHECK = "1";
